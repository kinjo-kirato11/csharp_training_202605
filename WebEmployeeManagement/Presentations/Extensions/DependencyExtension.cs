...
using WebEmployeeManagement.Applications.Services.Impls;
...
using WebEmployeeManagement.Presentations.Controllers;
using WebEmployeeManagement.Presentations.ViewModels;

namespace WebEmployeeManagement.Presentations.Extensions;
/// <summary>
/// 依存定義および依存性注入クラス
/// </summary>
    ...
    private static void SettingPresentations(IServiceCollection services)
    {
        ...
        services.AddScoped(
            ...
            new TempDataStore<EmployeeRegisterViewModel>("EmployeeRegisterViewModel")
                ...
                services.AddScoped<DepatmentRegisterViewModelAdapter>();
        // TempDataへのEmployeeRegisterViewの保存・復元するためのクラス
        // コンストラクタを利用して明示的にDIコンテナにインスタンスを登録する
        services.AddScoped(
            provider =>
